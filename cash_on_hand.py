...
def calculate_CashOnHand_difference(cash_on_hand):
    ...
    for value in range(1, len(cash_on_hand)):
        ...
        if current_cash_surplus < previous_cash_surplus:
            ...
            highest_cash_surplus = max(highest_cash_surplus, difference)

    # store highest cash surplus and any cash deficits into separate variables
    return highest_cash_surplus, cash_deficits


def print_cash_surplus(highest_cash_surplus):
    """
    ...
    print(f"[HIGHEST CASH SURPLUS]: AMOUNT: USD{highest_cash_surplus}")
...
highest_cash_surplus, cash_deficits = calculate_CashOnHand_difference(cash_on_hand)
...
# if there are no cash deficits where it keeps profitting, it will only print cash surplus and the day
    ...
    print_cash_surplus(highest_cash_surplus)